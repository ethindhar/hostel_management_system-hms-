...
CREATE TABLE registered_students (
    student_id INT PRIMARY KEY AUTO_INCREMENT,   -- Unique ID for the registered student
    roll_no INT UNIQUE NOT NULL,                 -- Student's roll number (must be unique)
    name VARCHAR(100) NOT NULL,                  -- Full name
    email VARCHAR(100) UNIQUE NOT NULL,          -- Email address
    password VARCHAR(255) NOT NULL,              -- Hashed password for security
    year INT NOT NULL,                           -- Year of study
    department VARCHAR(100) NOT NULL             -- Department
);


CREATE TABLE warden (
    warden_id INT AUTO_INCREMENT PRIMARY KEY, -- Unique ID for the warden
    name VARCHAR(100) NOT NULL,               -- Warden's full name
    email VARCHAR(100) UNIQUE NOT NULL,       -- Warden's email address
    phone_number VARCHAR(15) NOT NULL,        -- Contact number
    password VARCHAR(255) NOT NULL,           -- Hashed password
    hostel_id INT NOT NULL,                   -- Associated hostel
...
CREATE TABLE `failed_student` (
  ...
  `branch` enum('cse','me','eee','ai','ce','ec','bt','as','iem') NOT NULL,
...
CREATE TABLE `student` (
  ...
  `gender` enum('f','m') DEFAULT NULL,
  `address` varchar(200) DEFAULT NULL,
  `contact_no` VARCHAR(10) NOT NULL,  -- Updated to VARCHAR(10)
  `year` enum('1','2','3','4','0') NOT NULL,
  `branch` enum('cse','me','eee','ai','ec','bt','ce','as','iem') NOT NULL,
  `hostel_id` int(11) NOT NULL,
